...
FROM publishers, editions 
WHERE publishers.id = editions.publisher_id AND publishers.name = 'Random House';
...
SELECT stock.isbn, books.title, stock.stock, stock.retail,  
CASE 
  WHEN editions.type = 'h' THEN 'Hardback'
...
SELECT ROUND(AVG(cost),2) AS Average_cost, ROUND(AVG(retail),2) AS Average_Retail, ROUND((AVG(retail) - AVG(cost)),2) AS Average_profit 
FROM stock;
